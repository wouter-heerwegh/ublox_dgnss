...
namespace ublox_dgnss
{
	...
	public:
									  ...
									  .automatically_declare_parameters_from_overrides(true))
		{
			...
			RCLCPP_INFO_STREAM(this->get_logger(), "Setting up ROS");
			...
			RCLCPP_INFO_STREAM(this->get_logger(), "Done setting up ROS, testing out usb connection");

			try
			{
				usbc_ = std::make_shared<usb::Connection>(F9P_VENDOR_ID, F9P_PRODUCT_ID);
				RCLCPP_INFO_STREAM(this->get_logger(), "Got through making usb::Connection");
				...
				RCLCPP_INFO_STREAM(this->get_logger(), "LIBUSB API VERSION: " << std::hex << LIBUSB_API_VERSION);
				usbc_->init();
				RCLCPP_INFO_STREAM(this->get_logger(), "Got through making usb init");

				usbc_->init_async();
				RCLCPP_INFO_STREAM(this->get_logger(), "Got through making usb async init");
			}
			...
			RCLCPP_INFO_STREAM(this->get_logger(), "USB set up");
